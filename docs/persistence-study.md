...
4. **Amendment (verifier-caught, then measured):** a coroutine that yielded from inside a compiled trace sits under a `cont_stitch` frame whose aux slot holds a `GCtrace` reference — common on OC's hot path, not an edge case. On persist, write a **zero-payload trace reference** (not nil); the VM's own blacklist path then degrades resumption to `cont_nop`/interpreter — the same mechanism `jit.flush` relies on. **M0 located the slot empirically: `framebase - 5`**, one below the continuation pair ([prototype/framewalk](../prototype/framewalk/)). Hook-yield frames (raw multres integer) are refused, matching Eris's own "cannot persist yielded hooks."
